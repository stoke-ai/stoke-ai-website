...
test('opens a converted estimate through its verified job record route',()=>assert.equal(selectHcpEstimateOptionUrl([{id:'job_46c37bdcdfbc45fa8634658bdbbc0a02',total_amount:221500}],2215),'https://pro.housecallpro.com/app/estimates/job_46c37bdcdfbc45fa8634658bdbbc0a02'));
test('selects the only provider-approved same-price option',()=>assert.equal(selectHcpEstimateOptionUrl([{id:'est_0cf6607ab6d1487c9a183330dc95238c',total_amount:608500,approval_status:'pro declined'},{id:'est_ee538975381042208a84dcb91193ac7a',total_amount:608500,approval_status:'pro approved'}],6085),'https://pro.housecallpro.com/app/estimates/est_ee538975381042208a84dcb91193ac7a'));
test('still fails closed for same-price options without one approved choice',()=>assert.equal(selectHcpEstimateOptionUrl([{id:'est_11111111111111111111111111111111',total_amount:100,approval_status:'expired'},{id:'est_22222222222222222222222222222222',total_amount:100,approval_status:'expired'}],1),undefined));
