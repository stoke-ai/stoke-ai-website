...
test("door request labels follow Braxton's size, obstruction, color, panel, windows, style, and glass list", async () => {
  ...
  assert.match(form, /panel-faces\/raised\.webp/);
  assert.match(form, /panel-faces\/flush\.webp/);
  assert.match(form, /panel-faces\/carriage\.webp/);
  assert.match(form, /panel-faces\/glass\.webp/);
  assert.match(form, /Raised panel/);
  assert.match(form, /Carriage-house look/);
  assert.match(form, /Full-view glass/);
  assert.doesNotMatch(form, /Insulated panel/);
  assert.doesNotMatch(form, /No photo yet/);
  assert.doesNotMatch(form, /modern-tech-3550|therma-tech-3400|ap200n\.webp/);
  assert.match(form, /8x8/);
  assert.match(form, /10x8/);
  assert.match(form, /16x8/);
  assert.match(form, /10x12/);
  assert.match(form, /18x14/);
  assert.match(form, /Almond/);
  assert.match(form, /Sandstone/);
  assert.match(form, /Gray/);
  assert.match(form, /Tan/);
  assert.match(form, /Black/);
  assert.doesNotMatch(form, /Bronze|Charcoal|Hunter green/);
