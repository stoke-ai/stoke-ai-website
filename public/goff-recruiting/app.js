...
function showDraft(stage){ let x=c(); const stageTpl=stageMeta(stage).template; document.getElementById('modal').className='modal open'; document.getElementById('modal').innerHTML=`<div class="modal-card"><h3>Email the candidate</h3><p>Defaults to the template for their current stage — switch to any other Goff template below. Review or edit, then open it pre-filled in Gmail (or your mail app) and hit Send. It sends from your own careers@ account — replies come back to you.</p><label class="muted small" style="display:block;font-weight:700;margin-bottom:6px">Template</label><select class="stage-select" style="margin-bottom:12px" onchange="document.querySelector('.modal-card textarea').value=mergeKey(this.value,c())">${Object.keys(TEMPLATE_TEXT).map(k=>`<option ${k===stageTpl?'selected':''}>${esc(k)}</option>`).join('')}</select><textarea>${merge(stage,x)}</textarea><div class="modal-actions"><button class="btn" onclick="document.getElementById('modal').className='modal'">Close</button><button class="btn" onclick="copyToClipboard(document.querySelector('.modal-card textarea').value)">Copy</button><button class="btn" onclick="sendCandidateEmail('mailto')">Open in email app</button><button class="btn brand" onclick="sendCandidateEmail('gmail')">Open in Gmail →</button></div></div>`; }
