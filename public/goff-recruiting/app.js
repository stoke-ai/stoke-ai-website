...
function normalizeCandidate(x){
  ...
  x.offer = Object.assign({date:new Date().toISOString().slice(0,10), pay:'', startDate:'', schedule:'', supervisor:'', employmentType:'Full-Time', minHours:'40', validityDays:'30', notes:''}, x.offer || {});
...
function offer(){
  ...
  <div class="grid two" style="margin-top:16px">
    <section class="panel">
      <div class="section-head"><h3>Offer details</h3><span class="muted">Verified by the hiring manager before sending</span></div>
...
const OFFER_REQUIRED = [['date','Offer date','offerDate'],['startDate','Expected start date','offerStart'],['supervisor','Supervisor','offerSupervisor'],['pay','Starting pay','offerPay'],['minHours','Minimum hours','offerMinHours'],['schedule','Scheduled work hours','offerSchedule']];
...
function offerStepsHTML(o, x){
  ...
  return step(missing.length===0,'1. Verify request', missing.length===0?'All required offer fields are filled in.':`Still needed: ${missing.map(esc).join(', ')}.`)
    ...
    + `<div class="step ${o.signaturesRouted?'done':''}"><b>${o.signaturesRouted?'✓ ':''}4. Route signatures</b><br><small>Upload generated PDF/DOC to DocHub and assign the signature fields.</small><br><button class="btn step-btn" onclick="toggleSignaturesRouted()">${o.signaturesRouted?'Undo':'Mark signatures routed'}</button></div>`;
}
