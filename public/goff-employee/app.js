...
function setCourseSlide(i){ courseIndex = Math.max(0, Math.min(orientationSlides.length-1, i)); slideZoomOpen = false; render(); window.scrollTo({top:0, behavior:'smooth'}); }
function openSlideZoom(){ slideZoomOpen = true; render(); }
function closeSlideZoom(){ slideZoomOpen = false; render(); }
...
function courseSection(){
  ...
  const completeCount = orientationSlides.filter((_,i)=>completed[`course-${i}`]).length;
  return `<section class="course-player"><div class="course-top"><div><p class="eyebrow">First day employee orientation</p><h2>Goff onboarding course</h2><p>Slide ${courseIndex+1} of ${orientationSlides.length} • ${completeCount} complete</p></div><div class="course-meter"><strong>${coursePct()}%</strong><span>complete</span></div></div><div class="bar course-bar"><i style="width:${coursePct()}%"></i></div><article class="slide-viewer"><button class="slide-frame" onclick="openSlideZoom()" aria-label="Open slide ${courseIndex+1} larger"><img src="${esc(slide.src)}" alt="Goff Welding onboarding orientation slide ${courseIndex+1} of ${orientationSlides.length}" /><span>Tap to enlarge</span></button></article><div class="mobile-course-card"><b>Phone reading tip</b><p>This deck was built in landscape. Tap the slide to enlarge it, or turn the phone sideways for the clearest reading.</p></div><div class="course-actions"><button class="secondary" onclick="setCourseSlide(${courseIndex-1})" ${courseIndex===0?'disabled':''}>← Previous</button><button class="complete-btn ${done?'done':''}" onclick="toggleCourseSlide(${courseIndex})">${done?'Complete ✓':'Mark complete'}</button><button onclick="setCourseSlide(${courseIndex+1})" ${courseIndex===orientationSlides.length-1?'disabled':''}>Next →</button></div><div class="course-rail" aria-label="Course slides">${orientationSlides.map((s,i)=>`<button class="rail-dot ${i===courseIndex?'active':''} ${completed[`course-${i}`]?'done':''}" onclick="setCourseSlide(${i})" title="Slide ${i+1}">${i+1}</button>`).join('')}</div><p class="course-note">Progress is saved on this device for review. Production will save per employee.</p>${slideZoomOpen?`<div class="slide-zoom" onclick="closeSlideZoom()"><button class="zoom-close" onclick="event.stopPropagation();closeSlideZoom()">Close</button><img src="${esc(slide.src)}" alt="Expanded Goff onboarding slide ${courseIndex+1}" /><p>Turn phone sideways for the clearest view.</p></div>`:''}</section>`;
}
...
function adminSection(){ return `<section class="panel"><p class="eyebrow">Internal review guide</p><h2>Questions for Goff/BBSI walkthrough</h2><p>Use this as the conversation guide before finalizing automation. Build the draft pages now; finalize routing/signatures/reminders after Goff confirms the actual process.</p><div class="handoff"><div><span>Employee</span><b>${esc(PROFILE.employeeName)}</b></div><div><span>Portal status</span><b>Training path drafted</b></div><div><span>BBSI</span><b>Boundary marked</b></div><div><span>Next admin action</span><b>Confirm routing + signoffs</b></div></div><div class="question-list">${adminQuestions.map(([topic,q])=>`<article><span>${esc(topic)}</span><b>${esc(q)}</b></article>`).join('')}</div><p class="note"><strong>Production database needed:</strong> reminders and status changes should write to the shared employee record before Goff uses this with real employees.</p><div class="admin-actions"><button disabled title="Requires production database">Generate welcome email</button><button disabled title="Requires production database">Send day-before reminder</button><button disabled title="Requires production database">Mark BBSI complete</button><button disabled title="Requires production database">Schedule 30-day check-in</button></div></section>`; }
...
function render(){
  ...
  if(section==='course'){
    ...
    app.innerHTML = `${header()}<main class="wrap">${tabs()}${main()}</main><footer>Private Goff Welding employee portal • Formal payroll/compliance remains with BBSI/myBBSI.</footer>`;
  }
}
render();
