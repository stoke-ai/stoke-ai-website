...
// can be reinstated in minutes if Dale wants a certification-gate quiz — wire it
...
function opsSection(){
  ...
  <div class="metric-grid">${(()=>{const s=kcStats();return [
    ...
    { label:'Safety sections', value:`${SAFETY_COURSES.filter(c=>courseComplete('safety',c)).length}/${SAFETY_COURSES.length}`, detail:'Final V3 quiz retired 7/2 — Dale may reinstate' },
...
function safetySection(){
  ...
  <section class="panel"><div class="confirm-box"><h3>Pending Dale / Goff</h3><ul>${p.questions.map(q=>`<li>${esc(q)}</li>`).join('')}</ul></div></section>`;
}
