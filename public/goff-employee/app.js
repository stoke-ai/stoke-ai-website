...
const ORIENTATION_STEPS = [
  { austin:true, theme:'dark', eyebrow:'First day employee orientation', title:'Welcome to Goff Welding',
    ...
    body:'On behalf of your colleagues, we welcome you to Goff Welding and wish you every success here. Every employee plays a direct role in our success — we expect you to take pride in your work and ownership in what we build together. We hope your experience here will be challenging, enjoyable, and rewarding.',
  ...
  { eyebrow:'Who we are', title:'Our mission and vision',
    cards:[
      ['Our vision','To be known for adding value — always — by taking ownership and standing behind everything we do.','eye'],
      ['Our mission','We add value every day — through integrity, respect, and continuous improvement.','flag'],
    ] },
  ...
  { eyebrow:'Our values', title:'How we carry ourselves',
    cards:[
      ['Integrity','Doing the right thing when nobody’s watching.','shield'],
      ['Humility','Be open to feedback and committed to continuous improvement.','cap'],
      ['Respect','Treat others how you would like to be treated.','users'],
      ['Accountability','Take ownership of your actions, your work, and the outcome.','clipboard'],
    ],
  ...
  { eyebrow:'Living the values', title:'Nine ways you live them', numbered:true,
    cards:[
      ...
      ['Be tough, but fair',''],
      ['Keep your word',''],
      ['Ride for the brand — represent Goff Welding with pride',''],
      ['Say less, do more',''],
      ['Communicate clearly and specifically',''],
      ['Know where to draw the line — integrity is not negotiable',''],
    ] },
  ...
  { austin:true, theme:'dark', eyebrow:'The standard we hold', title:'What Goff expects of you',
    body:'Five things: Reliability — show up on time, ready to work. Ownership — take responsibility for your work and outcomes. Work ethic — be productive and stay engaged. Communication — speak clearly, ask questions, and follow through. Pride — care about the quality of what you produce.',
...
const pageContent = {
  ...
  clearance: {
    ...
    blocks:[
      ['Drug screen','Pre-employment drug screening at WorkMed-Intermountain Health in Burley, 8:00 AM–4:00 PM. Per the master checklist: delays in completing the drug test push the first day of employment out.'],
  ...
  milestones: {
    ...
    blocks:[
      ...
      ['60 days','Insurance eligibility — confirmed by the new master Welcome packet (the older handbook says 3 months for Sterling; master wins as the newer document). Admin reminder to start enrollment.'],
...
const POLICY_LIST = [
  { name:'Employee Handbook (Employee Copy)', type:'Sign acknowledgement', who:'All employees', note:'Handbook Acknowledgement form exists in Drive as a separate signable. Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
  { name:'Drug & Alcohol Policy — Revised Random Testing', type:'Sign', who:'All employees', note:'Confirm the “Revised Random Testing” version is current. Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
  { name:'NDA / Confidentiality Agreement', type:'Sign', who:'All employees', note:'Secure, employee-specific record. Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
  { name:'Video Release Form', type:'Sign', who:'All employees', note:'Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
  ...
  { name:'Goff Welding Vehicle Policy', type:'Sign', who:'Drivers / vehicle users', note:'Per the doc manifest: the published PDF is authoritative; the two docx copies are pending confirmation as source files. Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
  ...
  { name:'Vacation Policy (Master)', type:'Read & acknowledge', who:'All employees', note:'Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
  { name:'Unexcused Absences Policy', type:'Read & acknowledge', who:'All employees', note:'Master checklist: signature collected via the BBSI onboarding link — portal shows status, BBSI owns the signature.' },
...
function onboardingCard(){
  if(onboardingComplete()){
    return `<section class="panel training-done-strip"><div class="progress-parts">${onboardingParts().map((p,i)=>`<span class="done"><b>${i+1}</b> ${esc(p.label)} ✓</span>`).join('')}<span class="done"><b>✓</b> Training complete</span></div></section>`;
  }
  const next = firstIncompletePathStep();
  const pctNow = onboardingPct();
  return `<section class="onboard-card"><span class="g-mark" aria-hidden="true">G</span><div class="onboard-card-in">
    <p class="slide-eyebrow">Your onboarding — ${pctNow}% complete</p>
    <h2>Next up: ${esc(next ? next.label : 'Finish up')}</h2>
    <div class="bar onboard-bar"><i style="width:${pctNow}%"></i></div>
    <div class="progress-parts onboard-parts">${onboardingParts().map((p,i)=>`<span class="${p.pct===100?'done':''}"><b>${i+1}</b> ${esc(p.label)} · ${p.pct}%</span>`).join('')}</div>
    <div class="onboard-meta"><span><b>First day:</b> ${esc(PROFILE.startDate)}, ${esc(PROFILE.startTime)}</span><span><b>Where:</b> ${esc(PROFILE.location)}</span><span><b>Supervisor:</b> ${esc(PROFILE.supervisor)}</span></div>
    <div class="hero-actions"><button onclick="nav('${next ? next.id : 'path'}')">Continue onboarding →</button><button class="secondary" onclick="nav('path')">See all 5 steps</button></div>
  </div></section>`;
}
function employeeHome(){
  ...
  return `${onboardingCard()}<section class="panel employee-path"><p class="eyebrow">Quick actions</p><h2>Do something</h2><div class="home-actions">${quick.map(([id,label,hint])=>`<button class="home-action" onclick="${id}"><b>${esc(label)}</b><small>${esc(hint)}</small></button>`).join('')}</div></section>
  ...
  <section class="grid two">${onboardingComplete()?`<article class="panel"><p class="eyebrow">My training record</p><h2>Complete ✓</h2><p>Your acknowledgements and signatures are stored on your employee record in production. A yearly refresher pulls 5 random sections; your manager can also assign retraining.</p><button class="secondary" onclick="nav('path')">Revisit my onboarding path</button></article>`:`<article class="panel"><p class="eyebrow">While you're onboarding</p><h2>These tools are already yours.</h2><p>Safety reporting, the FAQs, and help contacts never wait for training to finish. If something happens in the shop, report it — training status doesn't matter.</p><button class="secondary" onclick="openFormFill('nearmiss')">Report a near miss</button></article>`}<article class="panel"><p class="eyebrow">Stuck on something?</p><h2>Ask before guessing.</h2><p>Search the FAQs first - most answers are there. Then your supervisor, then the office. If you cannot reach your supervisor, call the main office line.</p><button class="secondary" onclick="nav('help')">Who to contact</button></article></section>`;
}
function pathSteps(){
  return [
...
function exaktimeSection(){
  ...
  ${allWorkBasicsDone()?`<div class="ack-box"><h3>Work basics complete ✓</h3><p>This step is done. The FAQ keeps every one of these answers searchable whenever you need a refresher.</p></div>${nextStepDoor('Work basics')}`:''}</section>`;
}
...
const HANDOFF_CHECKLIST = [
  ...
  ['Meet the Scheduler','Get Work Schedule access confirmed and know who to call if you lose it.'],
  ['ExakTime live run-through','Clock in and out once together, in the app, on your phone — so payday never surprises anyone.'],
...
function faqSection(){
  ...
  <div id="faq-results">${faqResults()}</div></section>`;
}
...
function roleSection(){
  ...
  <div class="doc-blocks" style="margin-top:16px"><article><h3>What success looks like</h3><p>${esc(k.success)}</p></article><article><h3>Failure signals</h3><p>${esc(k.failure)}</p></article></div></article></section>`;
}
